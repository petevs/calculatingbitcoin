  ...
  useEffect(() => {
    const updateResults = () => {
      ...
      if (details.capitalGain === true) {
        tax = Math.round(profit * 0.5 * (details.taxRate / 100));
        taxableGain = profit * 0.5;
        gainRate = 50;
      }
      ...
      const percentageLess = Math.round(
        (1 - breakEven / details.currentPrice) * 100
      );
    ...
    >
      ...
      <CalcContainer>
        ...
        <CalcColumn>
          ...
          <p>{`To end up with the same amount of bitcoin you started with, the buyback price of bitcoin will need to be: $${Math.round(
            results.breakEven
          )}. Requiring an ${results.percentageLess}% drop in price`}</p>
          <MyNumberFormat
            id="buyBackPrice"
            label="Buyback Price"
